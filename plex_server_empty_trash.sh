...
SHOWS_SECTION_ID=5
SHOWS_MOUNT_TEST="/path/to/media/Shows/.test.txt"
...
then
  ...
  curl -X PUT -H "X-Plex-Token: $PLEX_TOKEN" $PLEX_URL/library/sections/$MOVIE_SECTION_ID/emptyTrash
  echo "Waiting 15 seconds to proceed."
  ping -i 1 -c 15 127.0.0.1 >/dev/null 2>&1
else
...
then
  ...
  curl -X PUT -H "X-Plex-Token: $PLEX_TOKEN" $PLEX_URL/library/sections/$SHOWS_SECTION_ID/emptyTrash
  echo "Waiting 15 seconds to proceed."
  ping -i 1 -c 15 127.0.0.1 >/dev/null 2>&1
  echo "done."
else
